  ...
  rmm::mr::device_memory_resource* mr)
{
  ...
  if (has_nulls) {
    conditional_join_semi<DEFAULT_JOIN_BLOCK_SIZE, DEFAULT_JOIN_CACHE_SIZE, true>
                 ...
                 rmm::mr::device_memory_resource* mr)
{
  if (join_type == join_kind::LEFT_ANTI_JOIN || join_type == join_kind::LEFT_SEMI_JOIN) {
    return std::make_pair(
      std::move(conditional_join_semi(left, right, binary_predicate, join_type, stream, mr)),
